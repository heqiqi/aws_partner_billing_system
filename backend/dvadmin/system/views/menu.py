...
from dvadmin.system.models import Menu, MenuButton, Dept
...
class MenuViewSet(CustomModelViewSet):
    ...
    def dashboard_url(self, request):
        """用于前端获取当前用户所能展示的dashboard"""
        user = request.user
        if not user.is_superuser:
            dept = user.dept
            serializer = DeptSerializer(dept, many=False, request=request)
            data = [serializer.data]
        else:
            DeptQueryset = Dept.objects.all()
            serializer = DeptSerializer(DeptQueryset, many=True, request=request)
            data = serializer.data
